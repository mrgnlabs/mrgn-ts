  ...
  const handleSimulation = React.useCallback(
    async (amount: number) => {
      try {
        ...
        const repayCollatActionTxns = await fetchRepayActionTxns(props);

        if (repayCollatActionTxns.actionMessage || repayCollatActionTxns.actionTxns === null) {
          handleError(repayCollatActionTxns.actionMessage ?? STATIC_SIMULATION_ERRORS.REPAY_COLLAT_FAILED, {
            setErrorMessage,
            setSimulationResult,
            setActionTxns,
            setIsLoading,
        ...
        const simulationResult = await simulationAction({
          txns: [
            ...(repayCollatActionTxns?.actionTxns?.repayCollatObject?.additionalTxns ?? []),
            ...(repayCollatActionTxns?.actionTxns?.repayCollatObject?.actionTxn
              ? [repayCollatActionTxns?.actionTxns?.repayCollatObject?.actionTxn]
              : []),
          ],
