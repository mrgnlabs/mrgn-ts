...
// src/client.ts
import {
  AnchorProvider as AnchorProvider2,
  BorshAccountsCoder,
  Program,
  translateAddress as translateAddress3
...
var PYTH_PRICE_CONF_INTERVALS = new BigNumber(4.24);
var USDC_DECIMALS = 6;
...
function loadKeypair(keypairPath) {
  ...
  const loaded = Keypair2.fromSecretKey(
...
function wrappedI80F48toBigNumber({ value }, scaleDecimal = 0) {
  ...
  return new BigNumber2(numbers.toString());
}
...
function parseBankConfig(bankConfigRaw) {
  return {
    ...
    address: new PublicKey3(bankConfigRaw.address)
  };
}
function parseConfig(configRaw) {
  return {
    ...
    programId: new PublicKey3(configRaw.program),
    groupPk: new PublicKey3(configRaw.group),
...
// src/group.ts
import {
  BorshCoder as BorshCoder2,
  translateAddress as translateAddress2
} from "@project-serum/anchor";
import { parsePriceData as parsePriceData3 } from "@pythnetwork/client";
...
var MarginfiAccount = class {
  ...
  async deposit(amount, bank) {
    ...
    const tx = new Transaction3().add(...ixs.instructions);
  ...
  getHealthComponents(marginReqType) {
    const [assets, liabilities] = this._lendingAccount.map((accountBalance) => {
      ...
      [new BigNumber3(0), new BigNumber3(0)]
    );
  ...
  getBalance(bankPk) {
    return this._lendingAccount.find((b) => b.bankPk.equals(bankPk)) ?? Balance.newEmpty(bankPk);
  }
  getFreeCollateral() {
    const { assets, liabilities } = this.getHealthComponents(
      MarginRequirementType.Init
    );
    return BigNumber3.max(0, assets.minus(liabilities));
  }
  ...
  async lendingAccountLiquidate(liquidateeMarginfiAccount, assetBank, assetQuantityUi, liabBank) {
    ...
    const tx = new Transaction3().add(...ixw.instructions);
...
var Balance = class {
  ...
  static newEmpty(bankPk) {
    return new Balance({
      active: false,
      bankPk,
      depositShares: { value: new BN2(0) },
      liabilityShares: { value: new BN2(0) }
    });
  }
...
// src/bank.ts
import { parsePriceData as parsePriceData2 } from "@pythnetwork/client";
var Bank = class {
  ...
  async reloadPriceData(connection) {
    ...
    this.priceData = parsePriceData2(pythPriceAccount.data);
  }
  ...
  getUsdValue(quantity, priceBias, weight) {
    ...
    return quantity.times(price).times(weight ?? 1).dividedBy(10 ** this.mintDecimals);
  }
  getPrice(priceBias) {
    const basePrice = this.priceData.emaPrice;
    const confidenceRange = this.priceData.emaConfidence;
    const basePriceVal = new BigNumber4(basePrice.value);
    const confidenceRangeVal = new BigNumber4(confidenceRange.value).times(
  ...
  getInterestRates() {
    ...
    const borrowingRate = interestRate.times(new BigNumber4(1).plus(rateFee)).plus(fixedFee);
  ...
  interestRateCurve() {
    ...
    if (utilizationRate.lte(optimalUtilizationRate)) {
      ...
      return utilizationRate.minus(optimalUtilizationRate).div(new BigNumber4(1).minus(optimalUtilizationRate)).times(maxInterestRate.minus(plateauInterestRate)).plus(plateauInterestRate);
    }
  }
  getUtilizationRate() {
  ...
  static async fetch(config, program, commitment) {
    ...
    const banks = bankAccountsData.map(
      (bd, index) => new bank_default(
        ...
        bankAddresses[index],
        bd,
        parsePriceData3(pythAccounts[index].data)
      )
    );
...
var MarginfiClient2 = class {
  ...
  static async fetch(config, wallet, connection, opts) {
    ...
    return new MarginfiClient2(
      ...
      program,
      wallet,
      await group_default.fetch(config, program, opts?.commitment)
    );
  }
  static async fromEnv(overrides) {
    ...
    const programId = overrides?.programId ?? new PublicKey6(process.env.MARGINFI_PROGRAM);
    const groupPk = overrides?.marginfiGroup ?? (process.env.MARGINFI_GROUP ? new PublicKey6(process.env.MARGINFI_GROUP) : PublicKey6.default);
    const wallet = overrides?.wallet ?? new NodeWallet(
      process.env.MARGINFI_WALLET_KEY ? Keypair3.fromSecretKey(
  ...
  async makeCreateMarginfiAccountIx(marginfiAccountKeypair) {
    ...
    const accountKeypair = marginfiAccountKeypair || Keypair3.generate();
  ...
  async createMarginfiAccount(opts) {
    const dbg = require_src()("mfi:client");
    const accountKeypair = Keypair3.generate();
    const ixs = await this.makeCreateMarginfiAccountIx(accountKeypair);
    const tx = new Transaction4().add(...ixs.instructions);
  ...
  async processTransaction(transaction, signers, opts) {
    ...
    try {
      ...
      const versionedTransaction = new VersionedTransaction2(
...
import {
  AnchorProvider as AnchorProvider3,
  BorshAccountsCoder as BorshAccountsCoder2,
  Program as Program2,
  translateAddress as translateAddress5
} from "@project-serum/anchor";
...
import { parsePriceData as parsePriceData4 } from "@pythnetwork/client";
...
var MarginfiAccountReadonly = class {
  ...
  async reload() {
    ...
    const banks = bankAccountsData.map(
      (bd, index) => new bank_default(
        ...
        bankAddresses[index],
        bd,
        parsePriceData4(pythAccounts[index].data)
      )
    );
    this._group = group_default.fromAccountDataRaw(
      this._config,
  ...
  getHealthComponents(marginReqType) {
    const [assets, liabilities] = this._lendingAccount.map((accountBalance) => {
      ...
      [new BigNumber5(0), new BigNumber5(0)]
    );
    return { assets, liabilities };
  }
  canBeLiquidated() {
...
var MarginfiClientReadonly = class {
  ...
  static async fetch(config, connection, opts) {
    ...
    const provider = new AnchorProvider3(connection, {}, {
      ...AnchorProvider3.defaultOptions(),
      commitment: connection.commitment ?? AnchorProvider3.defaultOptions().commitment,
      ...opts
    });
  ...
  static async fromEnv(overrides) {
    const debug = require_src()("mfi:client");
    const env = overrides?.env ?? process.env.MARGINFI_ENV;
    const connection = overrides?.connection ?? new Connection3(process.env.MARGINFI_RPC_ENDPOINT, {
      commitment: DEFAULT_COMMITMENT
    });
    const programId = overrides?.programId ?? new PublicKey7(process.env.MARGINFI_PROGRAM);
    const groupPk = overrides?.marginfiGroup ?? (process.env.MARGINFI_GROUP ? new PublicKey7(process.env.MARGINFI_GROUP) : PublicKey7.default);
    debug("Loading the marginfi client from env vars");
  ...
  async makeCreateMarginfiAccountIx(marginfiAccountKeypair) {
    const dbg = require_src()("mfi:client");
    const accountKeypair = marginfiAccountKeypair || Keypair4.generate();
    dbg("Generating marginfi account ix for %s", accountKeypair.publicKey);
    const initMarginfiAccountIx = await instructions_default.makeInitMarginfiAccountIx(
      this.program,
      {
        ...
        filters: [
          ...
          },
          {
            memcmp: {
              offset: 0,
              bytes: bs582.encode(
                BorshAccountsCoder2.accountDiscriminator(
                  "marginfiAccount" /* MarginfiAccount */
                )
              )
            }
        ...
        filters: [
          {
            memcmp: {
              offset: 0,
              bytes: bs582.encode(
                BorshAccountsCoder2.accountDiscriminator(type)
              )
            }
          }
        ]
...
export {
  ...
  clientReadonly_default as MarginfiReadonlyClient,
  NodeWallet,
