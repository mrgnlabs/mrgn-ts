...
<Property name="repayWithCollateral" type="">
    ...
    Parameters:
    - `props.repayAmount` (`Amount`): The amount to repay of the borrowed asset
    - `props.withdrawAmount` (`Amount`): The amount of collateral to withdraw
    - `props.borrowBankAddress` (`PublicKey`): The address of the bank where the asset was borrowed from.
    - `props.depositBankAddress` (`PublicKey`): The address of the bank where the collateral is deposited.
    - `props.withdrawAll` (`boolean`): If true, withdraws all available collateral. Automatically assigns this based on the amount if not overridden.
    - `props.repayAll` (`boolean`): If true, repays the entire borrowed amount. Automatically assigns this based on the amount if not overridden. 
    - `props.swap` (`Object`): Configuration for the swap, including instructions and lookup tables.
    - `props.blockhash` (`string`): Optional recent blockhash
    - `props.withdrawOpts` (`MakeWithdrawIxOpts`): Optional configuration for the withdrawal process
    - `props.repayOpts` (`MakeRepayIxOpts`): Optional configuration for the repayment process
    - `props.processOpts` (`ProcessTransactionsClientOpts`): Optional transaction processing configuration
    - `props.txOpts` (`TransactionOptions`): Optional transaction options

    Returns a `Promise` that resolves to an array of transaction signatures as `TransactionSignature[]`.
  </Property>
...
<Property name="makeRepayWithCollatTx" type="">
    ...
    - `props.repayAll` (`boolean`): If true, repays the entire borrowed amount. Automatically assigns this based on the amount if not overridden.
    - `props.swap` (`Object`): Configuration for the swap, including instructions and lookup tables.
    - `props.blockhash` (`string`): An optional parameter for a recent blockhash.
    - `props.withdrawOpts` (`MakeWithdrawIxOpts`): Optional configuration for the withdrawal process.
    - `props.repayOpts` (`MakeRepayIxOpts`): Optional configuration for the repayment process.

    Returns a `Promise` that resolves to a `FlashloanActionResult` object containing additional and flashloan transactions.
  </Property>

<Property name="loop" type="">
    The `loopV2` method allows users to perform a looping operation, which involves borrowing and depositing assets to achieve a target leverage. It performs the following steps:
    ...
    Parameters:
    - `depositAmount` (`Amount`): The amount to deposit into the deposit bank.
    - `borrowAmount` (`Amount`): The amount to borrow from the borrow bank.
    - `depositBankAddress` (`PublicKey`): The address of the bank where the deposit is made.
    - `borrowBankAddress` (`PublicKey`): The address of the bank where the asset was borrowed from.
    - `swap` (`Object`): Configuration for the swap, including instructions and lookup tables.
    - `blockhash` (`string`): (Optional) Recent blockhash.
    - `depositOpts` (`MakeDepositIxOpts`): (Optional) Configuration for the deposit process.
    - `borrowOpts` (`MakeBorrowIxOpts`): (Optional) Configuration for the borrow process.
    - `setupBankAddresses` (`PublicKey[]`): (Optional) Setup banks addresses.
    - `processOpts` (`ProcessTransactionsClientOpts`): (Optional) Transaction processing configuration.
    - `txOpts` (`TransactionOptions`): (Optional) Transaction options.
...
<Property name="makeDepositIx" type="">
    ...
    Parameters:
    - `amount` (`Amount`): The amount of tokens to deposit, can be a number or Amount object.
    ...
    This method delegates the creation of deposit instructions to the `marginfiAccount` object.
  </Property>

<Property name="makeDepositTx" type="">
    The `makeDepositTx` method creates a transaction for depositing tokens into a Marginfi bank account.

    Parameters:
    - `amount` (`Amount`): The amount of tokens to deposit, can be a number or Amount object.
    - `bankAddress` (`PublicKey`): The address of the bank where the asset is being deposited.
    - `depositOpts` (`MakeDepositIxOpts`): (Optional) Configuration for creating the deposit instruction.

    Returns a `Promise` that resolves to an `ExtendedTransaction` object ready to be signed and sent.

    This method performs the following steps:
    1. Generates deposit instructions using `makeDepositIx`.
    2. Retrieves client address lookup table accounts.
    3. Adds transaction metadata including type, signers, and address lookup tables.
    4. Returns the prepared transaction object.
  </Property>
...
<Property name="deposit" type="">
    ...
    This method performs the following steps:
    1. Generates deposit instructions using `makeDepositIx`.
    2. Builds and processes the transaction.
    3. Returns the transaction signature upon successful completion.
  </Property>
...
<Property name="makeRepayIx" type="">
    ...
    Parameters:
    - `amount` (`Amount`): The amount of the asset to repay, can be a number or Amount object.
    ...
    - `repayOpts` (`MakeRepayIxOpts`): (Optional) Additional options for creating the repay instruction.
...
<Property name="makeRepayTx" type="">
    The `makeRepayTx` method creates a transaction for repaying a loan in a Marginfi account.
    ...
    Returns a `Promise` that resolves to an `ExtendedTransaction` object containing the repay instructions.

    This method performs the following steps:
    1. Generates repay instructions using `makeRepayIx`.
    2. Retrieves client address lookup table accounts.
    3. Adds transaction metadata including type, signers, and address lookup tables.
    4. Returns the prepared transaction object.
  </Property>
