...
marginfi primarily uses Pyth and Switchboard oracles to source token market prices, relying on Pyth’s robust safety features to ensure accurate pricing and effective risk management. For assets not supported by Pyth, the protocol falls back on Switchboard oracles, applying similar best practices to maintain consistency and reliability.
