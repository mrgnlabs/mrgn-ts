...
## Trading with Leverage {{ tag: '', label: '' }}

---

## Creating Permissionless Banks {{ tag: '', label: '' }}
...
Every bank belongs to a group, which acts as a container for banks, setting their configuration and administrative rules.

To create a bank within a group, your CLI profile must be assigned to that group.

<Note>
  If you have not already setup your CLI profile, refer to <Button href="https://docs.marginfi.com/rust-sdk" variant="text"><>the CLI docs</></Button> for guidance. **If you have already setup your CLI profile with a group, you can skip this step**.
</Note>
