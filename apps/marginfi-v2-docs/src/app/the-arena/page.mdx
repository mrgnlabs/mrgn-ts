...
**Taking a long or short position involves borrowing and lending different assets depending on the desired trade direction**. For example, if you want to open a long position for Token A, your goal is to profit from an increase in Token A's price, relative to Token B. To do this, you would borrow Token B and use it to purchase more of Token A. **All token banks on The Arena are paired with USDC**. Deposit a given bank's token to go long and leverage against USDC. Depsoit USDC to go short and leverage against the bank's token.
