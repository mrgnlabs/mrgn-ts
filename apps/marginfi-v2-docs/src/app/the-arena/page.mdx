...
import YieldExample from '~/images/docs/yield-example.png'
...
All token banks listed on The Arena are isolated and completely separate from those on mrgnlend. Banks need liquidity to enable trading. You can provide liquidity for an APY for both token and $USDC banks on a given bank's page. Select "Supply" on a bank card to get started.
...
For the time being, all token banks on The Arena are paired with $USDC. Deposit a given bank's token to go long and leverage against $USDC. Depsoit $USDC to go short and leverage against the bank's token. **Soon you will be able to create token pairs of any kind**.
...
**The yields for lending USDC and a given token are determined by the trading activity occuring within a given group**. Using the $BODEN group as an example:
...
The key factor driving the fluctuations in APY for lending and borrowing different assets is the trading activity and direction within a given group. When an asset is in high demand for borrowing (due to prevalent trading strategies), the platform incentivizes more users to supply that asset by offering higher yields. That being said, yields can fluctuate wildly, allowing arbitragers to take advantage of temporary imbalances in the market.
...
This is a massive unlock to new teams building crypto products. Any team in need of credit solutions can now use marginfi’s open-source, double-audited, code verified, stress-tested contracts while still being opinionated about the assets and collateral weights you want. **You can benefit from our liquidity network, our userbase, our liquidators, our risk systems, and our off-chain tooling**. 

Building credit infrastructure doesn’t let teams focus on the actual product they’re building. Now you don’t have to — immediately plug into a custom source of yield and leverage to suit your needs. There’s 20+ teams already doing this — if this is you, **reach out to @nathanzebedee on Telegram**.
