...
The marginfi protocol charges a 1% fixed rate on every borrow transaction and a 5% rate fee on borrow transactions when they cross a threshold. For example: if rate is 10% we would charge 0.5%.
