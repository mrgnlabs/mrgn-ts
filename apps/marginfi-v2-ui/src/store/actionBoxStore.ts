...
interface ActionBoxState {
    ...
    setSelectedBank: (tokenBankAddress: PublicKey, tokenBanks: ExtendedBankInfo[]) => void;
...
const stateCreator: StateCreator<ActionBoxState, [], []> = (set, get) => ({
    ...
    fetchActionBoxState(args) {
        ...
        let selectedBank: ExtendedBankInfo | null = null;
        ...
        if (args.requestedBank) {
            selectedBank = args.requestedBank;
        } else {
            selectedBank = null;
        }

        set({ ...initialState, actionMode, selectedBank });
    },
    ...
    async setSelectedBank(tokenBankAddress, tokenBanks) {
        const selectedBank = get().selectedBank;
        const hasBankChanged = !selectedBank || tokenBankAddress.equals(selectedBank.address);

        if (hasBankChanged) {
            const newTokenBank =
                tokenBanks.find((bank) => bank?.address?.equals && bank?.address?.equals(tokenBankAddress)) ?? null;
            set({ selectedBank: newTokenBank, amountRaw: "", amount: 0, repayAmountRaw: "", repayAmount: 0 });
    ...
    async setRepayTokenBank(tokenBankAddress, tokenBanks) {
        const selectedBank = get().selectedRepayBank;
        const hasBankChanged = !selectedBank || tokenBankAddress.equals(selectedBank.address);

        if (hasBankChanged) {
            const newRepayTokenBank =
                tokenBanks.find((bank) => bank?.address?.equals && bank?.address?.equals(tokenBankAddress)) ?? null;
            set({ selectedRepayBank: newRepayTokenBank, amountRaw: "", amount: 0, repayAmountRaw: "", repayAmount: 0 });
