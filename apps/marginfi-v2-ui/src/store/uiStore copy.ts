...
interface NonPersistentUiState {
  isActionBoxSearchOpen: boolean;
  setIsActionBoxSearchOpen: (isOpen: boolean) => void;
}

// Persistent State
const usePersistentUiStore = create<PersistentUiState>()(
  persist(
    (set, get) => ({
      // State
...
const useNonPersistentUiStore = create<NonPersistentUiState>((set) => ({
  isActionBoxSearchOpen: false,
  setIsActionBoxSearchOpen: (isOpen: boolean) => {
    console.log("isOpen", isOpen);
    set({ isActionBoxSearchOpen: isOpen });
  },
}));

// Create the store, merging both persistent and non-persistent state

export { usePersistentUiStore, useNonPersistentUiStore, SORT_OPTIONS_MAP };
export type { PersistentUiState, NonPersistentUiState };
