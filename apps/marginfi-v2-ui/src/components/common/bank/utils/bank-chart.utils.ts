...
export const computeInterestRates = ({
  utilizationRate,
  effectiveOptimalUtilization,
  effectivePlateauRate,
  effectiveMaxRate,
  effectiveInsuranceFeeFixedApr,
  effectiveProtocolFixedFeeApr,
  effectiveInsuranceIrFee,
  effectiveProtocolIrFee,
}: {
  utilizationRate: number;
  effectiveOptimalUtilization: number;
  effectivePlateauRate: number;
  effectiveMaxRate: number;
  effectiveInsuranceFeeFixedApr: number;
  effectiveProtocolFixedFeeApr: number;
  effectiveInsuranceIrFee: number;
  effectiveProtocolIrFee: number;
}): { lendingRate: number; borrowingRate: number } => {
  ...
  let baseInterestRate =
    utilizationRate <= effectiveOptimalUtilization
      ? (utilizationRate * effectivePlateauRate) / effectiveOptimalUtilization
      : ((utilizationRate - effectiveOptimalUtilization) / (1 - effectiveOptimalUtilization)) *
