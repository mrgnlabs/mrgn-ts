import React from "react";
import { useWallet, Wallet } from "@solana/wallet-adapter-react";
import { Adapter, WalletName, WalletAdapter, WalletReadyState } from "@solana/wallet-adapter-base";

import { useOs } from "~/hooks/useOs";
import { IconBackpackWallet } from "~/components/ui/icons";

type WalletPreset = "eth" | "sol" | "social";

const ETH_PRESET = ["Backpack", "Phantom", "WalletConnect", "MetaMask"];
const SOL_PRESET = ["Backpack", "Phantom", "Solflare"];
const SOCIAL_PRESET = ["Backpack", "Phantom", "Solflare"];

export function useAvailableWallets(preset?: WalletPreset) {
  const { wallets } = useWallet();
  const { isAndroid, isIOS } = useOs();

  const selectedPreset = React.useMemo(() => {
    if (preset === "eth") return ETH_PRESET;
    else if (preset === "sol") return SOL_PRESET;
    else if (preset === "social") return SOCIAL_PRESET;
  }, [preset]);

  const isBackpackInstalled = React.useMemo(
    () => !!wallets.find((wallet) => wallet.adapter.name === "Backpack"),
    [wallets]
  );

  /////// FILTERS & SORTERS
  // filter on preset
  const presetFilter = React.useCallback(
    (wallet: Wallet) => selectedPreset?.includes(wallet.adapter.name),
    [selectedPreset]
  );

  // filter on installed
  const installedFilter = React.useCallback(
    (wallet: Wallet) => wallet.readyState === "Installed" || wallet.readyState === "Loadable",
    []
  );

  // sorting on backpack
  const backpackSort = React.useCallback((a: Wallet, b: Wallet) => {
    if (a.adapter.name === "Backpack" && b.adapter.name !== "Backpack") {
      return -1;
    } else if (a.adapter.name !== "Backpack" && b.adapter.name === "Backpack") {
      return 1;
    }
    return 0;
  }, []);

  const bab = React.useMemo(() => {
    console.log(wallets.map((wallet) => wallet.adapter));
  }, [wallets]);

  const filteredWallets = React.useMemo(() => {
    let formattedWallet = wallets;
    if (selectedPreset) formattedWallet = formattedWallet.filter(presetFilter);

    if (preset !== "social") formattedWallet = formattedWallet.filter(installedFilter);

    if (preset === "social" && !isBackpackInstalled) formattedWallet = [...formattedWallet, backpackAdapter];

    formattedWallet = formattedWallet.sort(backpackSort);
    return formattedWallet;
  }, [backpackSort, installedFilter, preset, presetFilter, selectedPreset, wallets, isBackpackInstalled]);

  return filteredWallets;
}

type WalletInstallMap = { [wallet: string]: string };

export const walletInstallMap: WalletInstallMap = {
  Phantom: "https://phantom.app/ul/browse/https://app.marginfi.com?ref=https://app.marginfi.com",
  Backpack: "https://backpack.app/download",
  Solflare: "https://gamma.solflare.com/download#extension",
};

export const BackpackWalletName = "Backpack" as WalletName<"Backpack">;

const backpackAdapter: Wallet = {
  adapter: {
    name: BackpackWalletName,
    icon: "data:image/png;base64,",
    readyState: WalletReadyState.NotDetected,
  } as any,
  readyState: WalletReadyState.NotDetected,
};
