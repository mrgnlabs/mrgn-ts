...
export const SwapForm: React.FC<{
  onSubmit: () => void;
  isDisabled: boolean;
  setSelectPairSelector: React.Dispatch<
    React.SetStateAction<"fromMint" | "toMint" | null>
  >;
  ...
  const marketRoutes = selectedSwapRoute
    ? selectedSwapRoute.marketInfos.map(({ label }) => label).join(", ")
    : "";
  ...
  return (
    <View>
      ...
      {routes ? (
        <View
          style={tw`flex flex-row items-center mt-2 text-xs space-x-1 gap-4px`}>
          <Pressable
          ...
          <Text
            style={tw`text-white/50 overflow-hidden whitespace-nowrap text-ellipsis max-w-[70%]`}>
            {marketRoutes}
          </Text>
        </View>
      ) : (
