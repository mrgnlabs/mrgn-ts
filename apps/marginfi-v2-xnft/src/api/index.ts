...
import {
  ...
  TokenPriceMap,
  UserPosition,
} from "~/types/mrgnTypes";
import { WSOL_MINT } from "~/config";
import { floor } from "~/utils";
import { MarginfiAccount } from "~/lib/marginfi-client";
...
function computeAccountSummary(
  ...
  const equityComponentsUnbiased =
    marginfiAccount.getHealthComponentsWithoutBias(
      MarginRequirementType.Equity,
    );
  const equityComponentsWithBiasAndWeighted =
  ...
  if (uxpBalance) {
    outstandingUxpEmissions = uxpBalance
      .getTotalOutstandingEmissions(uxpBank!.bank)
  ...
  return {
    ...
    balanceUnbiased: equityComponentsUnbiased.assets
      .minus(equityComponentsUnbiased.liabilities)
      .toNumber(),
    ...
    lendingAmountWithBiasAndWeighted:
      equityComponentsWithBiasAndWeighted.assets.toNumber(),
    borrowingAmountWithBiasAndWeighted:
      equityComponentsWithBiasAndWeighted.liabilities.toNumber(),
...
function makeBankInfo(
  ...
  const utilizationRate =
    totalPoolDeposits > 0 ? (totalPoolBorrows / totalPoolDeposits) * 100 : 0;
  ...
  if (bank.emissionsActiveLending || bank.emissionsActiveBorrowing) {
    ...
    const emissionsRateAmount = new BigNumber(
      nativeToUi(bank.emissionsRate, emissionsTokenData.decimals),
    );
    const emissionsRateValue = emissionsRateAmount.times(
      emissionsTokenData.price,
    );
  ...
  return {
    ...
    borrowingRate: isNaN(borrowingRate.toNumber())
      ? 0
      : borrowingRate.toNumber(),
    emissionsRate,
    emissions,
    totalPoolDeposits,
    totalPoolBorrows,
...
async function fetchBirdeyePrice(tokenPubkey: PublicKey): Promise<BigNumber> {
  const response = await fetch(
    `${BIRDEYE_API}/public/price?address=${tokenPubkey.toBase58()}`,
    {
...
export async function buildEmissionsPriceMap(
  banks: Bank[],
  connection: Connection,
): Promise<TokenPriceMap> {
  const banksWithEmissions = banks.filter(
    (bank) => !bank.emissionsMint.equals(PublicKey.default),
  );
  ...
  const prices: { mint: PublicKey; price: BigNumber; decimals: number }[] =
    await Promise.all([...emissionsPrices, ...bankPrices]);
...
function makeExtendedBankInfo(
  ...
  const positionRaw = marginfiAccount?.activeBalances.find((balance) =>
    balance.bankPk.equals(bankInfo.address),
  ) as any;
  const hasActivePosition = !!positionRaw;
  const position = hasActivePosition
    ? makeUserPosition(positionRaw, bankInfo)
    : null;
  ...
  const maxBorrow = floor(
    Math.min(
      (marginfiAccount?.getMaxBorrowForBank(bankInfo.bank).toNumber() ?? 0) *
        0.95,
...
function makeUserPosition(balance: Balance, bankInfo: BankInfo): UserPosition {
  ...
  const usdValues = balance.getUsdValue(
    bankInfo.bank,
    MarginRequirementType.Equity,
  );
  ...
  return {
    ...
    weightedUSDValue: isLending
      ? weightedUSDValues.assets.toNumber()
      : weightedUSDValues.liabilities.toNumber(),
    isLending,
  };
}
