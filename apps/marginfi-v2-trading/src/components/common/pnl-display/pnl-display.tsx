...
export const PnlDisplay = ({ pnl, depositsAmountUsd, borrowsAmountUsd }: PnlDisplayProps) => {
  return (
    <Card className="py-2">
      ...
      <CardContent className="px-4 py-2">
        <div className="flex flex-col gap-2 bg-muted rounded-md px-4 py-2 text-sm">
          <div className="flex flex-row items-center justify-between ">
            ...
            <span className={`${depositsAmountUsd > 0 ? "text-success" : "text-error"}`}>
              {`${depositsAmountUsd > 0 ? "+" : "-"}$${dynamicNumeralFormatter(Math.abs(depositsAmountUsd), {
                minDisplay: 0.0001,
                maxDisplay: 100000,
              })}`}
            </span>
          ...
          <div className="flex flex-row items-center justify-between">
            <span className="text-muted-foreground">Borrows</span>
            <span className={`${borrowsAmountUsd > 0 ? "text-success" : "text-error"}`}>
              {`${borrowsAmountUsd > 0 ? "+" : "-"}$${dynamicNumeralFormatter(Math.abs(borrowsAmountUsd), {
                minDisplay: 0.0001,
                maxDisplay: 100000,
              })}`}
            </span>
