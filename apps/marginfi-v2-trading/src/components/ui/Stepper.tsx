...
export const StepperWrapper = React.forwardRef<HTMLDivElement, StepperProps & React.HTMLAttributes<HTMLDivElement>>(
  (
    {
      initialStep,
      steps,
...
export const Stepper = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  (
    {
      children,
    ...
    const items = React.Children.toArray(children).map((child, index) => {
      ...
      if (child.type !== StepperItem && child.type !== StepperFooter) {
        throw new Error("Stepper children must be either <StepperItem> or <StepperFooter>.");
      }
    ...
    return (
      <div ref={ref} className={cn("space-y-4", className)} {...props}>
        <div className={cn("flex w-full flex-1 items-start justify-between gap-2 text-center mb-10 flex-row")}>
          {items}
        </div>
...
const icons = {
  success: <Check className="h-6 w-6 text-black" />,
    ...
    >
      ...
      >
        <Button
          ...
          onClick={onClickItem}
          className={cn(
            "stepper-item-button aspect-square h-10 w-10 rounded-full p-2",
