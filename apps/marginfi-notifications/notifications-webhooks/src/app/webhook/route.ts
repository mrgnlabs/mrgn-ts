...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const healthThreshold = parseFloat(process.env.HEALTH_FACTOR_THRESHOLD || "0.25");

    if (isNaN(health) || health > healthThreshold) {
    ...
    if (!userData.account_health) {
      return new Response(JSON.stringify({ error: "User has account health notifications turned off" }), {
        status: 200,
      });
    }
    ...
    const lastNotificationTime =
      userData.last_notification instanceof admin.firestore.Timestamp
        ? userData.last_notification.toDate().getTime()
        : 0;
